...
def index():
    ...
    for g in goals:
        ...
        try:
            funds_raw = conn.execute(
                "SELECT amount, added_at FROM goal_funds WHERE goal_id=? ORDER BY added_at DESC",
                (g['id'],)
            ).fetchall()
            goal_dict['fund_history'] = [dict(f) for f in funds_raw]
        except Exception:
            goal_dict['fund_history'] = []
    ...
    return render_template('goals.html', goals=goals_data, monthly_budget=monthly_budget,
                           cm_expense=cm_expense, budget_used_percent=budget_used_percent)
...
@goals_bp.route('/goals/<int:id>/edit', methods=['POST'])
def edit_goal(id):
    if 'user_id' not in session:
        return redirect(url_for('auth.login'))
    ...
    conn.execute(
        ...
        (name, target_amount, deadline, id, uid)
    )
    conn.commit()
